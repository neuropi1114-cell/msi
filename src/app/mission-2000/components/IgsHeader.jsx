...
const navItems = [
  {
    label: 'The Mission', theme: 'aqua',
    children: [
      { label: 'About the Mission', url: '#', items: [
        { label: 'Our Vision', url: '#' }, { label: 'What We Do', url: '#' }, { label: 'Impact & Goals', url: '#' },
      ]},
      { label: 'Get Involved', url: '#', items: [
        { label: 'Volunteer', url: '#' }, { label: 'Donate', url: '#' }, { label: 'Partner With Us', url: '#' },
      ]},
    ],
  },
  {
    label: 'National ECCE Day', theme: 'yellow',
    children: [
      { label: 'About ECCE Day', url: '#', items: [
        { label: 'What is ECCE', url: '#' }, { label: 'Why It Matters', url: '#' }, { label: 'Events Near You', url: '#' },
      ]},
      { label: 'Participate', url: '#', items: [
        { label: 'Register Your Event', url: '#' }, { label: 'Resources', url: '#' }, { label: 'Share Your Story', url: '#' },
      ]},
    ],
  },
  {
    label: 'Join the Movement', theme: 'pink',
    children: [
      { label: 'Ways to Join', url: '#', items: [
        { label: 'Sign Up', url: '#' }, { label: 'Become an Advocate', url: '#' }, { label: 'Volunteer', url: '#' },
      ]},
      { label: 'Resources', url: '#', items: [
        { label: 'Toolkits', url: '#' }, { label: 'Campaign Materials', url: '#' }, { label: 'Success Stories', url: '#' },
      ]},
    ],
  },
  {
    label: 'For Schools', theme: 'orange',
    children: [
      { label: 'School Programs', url: '#', items: [
        { label: 'Programs Overview', url: '#' }, { label: 'Curriculum', url: '#' }, { label: 'Partner With Us', url: '#' },
      ]},
      { label: 'Resources', url: '#', items: [
        { label: 'Teacher Guides', url: '#' }, { label: 'Classroom Activities', url: '#' }, { label: 'Case Studies', url: '#' },
      ]},
    ],
  },
  {
    label: 'For Teachers', theme: 'green',
    children: [
      { label: 'Professional Learning', url: '#', items: [
        { label: 'Professional Development', url: '#' }, { label: 'Workshops', url: '#' }, { label: 'Webinars', url: '#' },
      ]},
      { label: 'Teaching Resources', url: '#', items: [
        { label: 'Lesson Plans', url: '#' }, { label: 'Activity Sheets', url: '#' }, { label: 'Community Forum', url: '#' },
      ]},
    ],
  },
  {
    label: 'For Parents', theme: 'aqua',
    children: [
      { label: 'Parent Programs', url: '#', items: [
        { label: 'Information Sessions', url: '#' }, { label: 'Workshops', url: '#' }, { label: 'Parent Resources', url: '#' },
      ]},
      { label: 'Support', url: '#', items: [
        { label: 'FAQs', url: '#' }, { label: 'Contact Us', url: '#' }, { label: 'Community Group', url: '#' },
      ]},
    ],
  },
  {
    label: 'Partners', theme: 'yellow',
    children: [
      { label: 'Our Partners', url: '#', items: [
        { label: 'Current Partners', url: '#' }, { label: 'Success Stories', url: '#' }, { label: 'Impact Report', url: '#' },
      ]},
      { label: 'Partner With Us', url: '#', items: [
        { label: 'Become a Partner', url: '#' }, { label: 'Sponsorship', url: '#' }, { label: 'Collaborate', url: '#' },
      ]},
    ],
  },
  {
    label: 'Stories', theme: 'pink',
    children: [
      { label: 'Latest Stories', url: '#', items: [
        { label: 'Blog', url: '#' }, { label: 'News', url: '#' }, { label: 'Media Coverage', url: '#' },
      ]},
      { label: 'Testimonials', url: '#', items: [
        { label: 'Parent Stories', url: '#' }, { label: 'Teacher Stories', url: '#' }, { label: 'Student Stories', url: '#' },
      ]},
    ],
  },
  {
    label: 'Contact', theme: 'orange',
    children: [
      { label: 'Get in Touch', url: '#', items: [
        { label: 'Contact Us', url: '#' }, { label: 'Locations', url: '#' }, { label: 'Office Hours', url: '#' },
      ]},
      { label: 'Support', url: '#', items: [
        { label: 'Help Center', url: '#' }, { label: 'Technical Support', url: '#' }, { label: 'Feedback', url: '#' },
      ]},
    ],
  },
];
      ...
      <div className="main-header_wrapper">
        <div className="logo">
          <a href="https://www.igssyd.nsw.edu.au/" title="International Grammar School Sydney">
            <svg className="igs-logo" width="80" height="80" viewBox="0 0 600 602" fill="none" xmlns="http://www.w3.org/2000/svg" style={{ maxWidth: '100%', height: 'auto' }}>
            ...
            <nav role="navigation" aria-label="primary">
              <ul>
                {navItems.map((item) => (
                    <li
                    key={item.label}
                    className={`first-level-item menu-theme-${item.theme} has-children`}
                  >
                    <a
                      tabIndex="0"
                      href="#"
                      className={activeDropdown === item.label ? 'active' : ''}
                      onClick={(e) => { e.preventDefault(); toggleDropdown(item.label); }}
                    >
                      <span className="icon"></span>
                      <span>{item.label}</span>
                    </a>
                    <ul className="child">
                      <li className="child-title"><span onClick={() => setActiveDropdown(null)}>{item.label}</span></li>
        ...
        <div className="header-secondary">
          ...
          <div className="search-menu">
            <a href="#" aria-label="Search - opens interface" className="search button small yellow" onClick={(e) => { e.preventDefault(); setSearchOpen(!searchOpen); }}>
              <i className="material-icons search">search</i>
            </a>
            <div className={`search-menu-overlay${searchOpen ? ' active' : ''}`}>
